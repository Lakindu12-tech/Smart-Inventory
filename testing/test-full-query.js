...
async function testFullQuery() {
  ...
  transactions.forEach(tx => {
    console.log(`Transaction ${tx.id}: Rs.${tx.total_amount} on ${tx.date.toISOString()}`);
    tx.items.forEach(item => {
      console.log(`  - ${item.product_name}: ${item.quantity} x Rs.${item.price}`);
    });
  });
  console.log('');
  ...
  console.log('📊 Average:', totalSales / transactions.length);
